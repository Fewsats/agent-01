document.addEventListener("DOMContentLoaded", () => {
    ...
    function displayBalanceInfo(
        finalBalance,
        balanceDifferenceUSD,
        balanceDifferenceSats,
    ) {
        console.log(
            "Displaying balance info:",
            finalBalance,
            balanceDifferenceUSD,
            balanceDifferenceSats,
        );
        const costDisplay =
            balanceDifferenceSats > 0
                ? ` <span class="text-red-500 ml-1">(-$${parseFloat(balanceDifferenceUSD).toFixed(2)})</span>`
                : "";
        walletBalanceElement.innerHTML = `Wallet: ${finalBalance}${costDisplay}`;
    }
    ...
    function autoResizeTextarea() {
        ...
        const lineHeight = parseInt(
            window.getComputedStyle(questionInput).lineHeight,
        );
    ...
    form.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.log(
                    "Received balance data:",
                    data.final_balance,
                    data.balance_difference_usd,
                    data.balance_difference_sats,
                );
                displayBalanceInfo(
                    data.final_balance,
                    data.balance_difference_usd,
    ...
    sendSmsButton.addEventListener("click", () => {
        setPrompt(
            "Tool to send SMS: l402://api.fewsats.com/v0/gateway/d4a9eff9-991f-4664-ab0e-d9add4597c76/info",
        );
    });

    accessWebsiteButton.addEventListener("click", () => {
        setPrompt(
            "Tool to scrape websites: l402://api.fewsats.com/v0/gateway/f12e5deb-b07b-4af4-a4f2-3fbf076228a9/info",
        );
    });
